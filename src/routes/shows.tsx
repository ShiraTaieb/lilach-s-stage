...
interface UpcomingShow { id: string; title: string; event_date: string; venue: string | null; description: string; image_url: string | null }
...
function ShowsPage() {
  ...
  useEffect(() => {
    supabase.from("upcoming_shows").select("*").order("event_date").then(({ data }) => setUpcoming(data ?? []));
    supabase.from("booking_shows").select("*").order("sort_order").then(({ data }) => setBooking(data ?? []));
  }, []);
            ...
            {upcoming.map((s) => (
              <div key={s.id} className="bg-gradient-card overflow-hidden rounded-3xl border border-primary/20 shadow-elegant">
                <div className="relative h-48 overflow-hidden">
                  <img src={s.image_url || gallery1} alt={s.title} loading="lazy" className="h-full w-full object-cover" />
          ...
          {booking.map((s, i) => (
            <div key={s.id} className="bg-gradient-card overflow-hidden rounded-3xl border border-border/60">
              <div className="aspect-video overflow-hidden">
                <img src={s.image_url || (i % 2 ? gallery2 : gallery1)} alt={s.title} loading="lazy" className="h-full w-full object-cover" />
              </div>
              <div className="p-6">
